...
resource "google_container_cluster" "mottu_dev_cluster" {
  ...
  ##Recurso deleta o cluster
  #resource_usage_export_config {
  #  enable_network_egress_metering = true
  #  enable_resource_consumption_metering = true

  #  bigquery_destination {
  #    dataset_id = "mottu_gke_resources"
  ## }
  #}
...
# Node Pool Gerenciado Separadamente loki
resource "google_container_node_pool" "nodes_mottu_loki" {
  name       = "${google_container_cluster.mottu_dev_cluster.name}-mottuloki-nodepool"
  location   = var.region
  cluster    = google_container_cluster.mottu_dev_cluster.name
  node_count = var.gke_num_nodes_loki
  autoscaling{
    min_node_count = var.gke_autoscaling_nodes_min
    max_node_count = var.gke_autoscaling_nodes_max
  }
